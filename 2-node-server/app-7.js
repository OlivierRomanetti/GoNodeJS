...
const server = http.createServer((req, res) => {
  ...
  if (url === '/register' && method === 'POST') {
    const fullData = [];
    // le "data event" est émis à chaque fois qu'un chunk de data est disponible à la lecture
    // La function passée en second paramètre est donc éxécutée à chaque fois que le data event est émis
    // JavaScript nous passe en premier argument le chunk de data correspondant
    req.on('data', (chunk) => {
    ...
    req.on('end', () => {
      ...
      fs.writeFileSync('extraterrestrials-7.txt', extraterrestrials);
    });
