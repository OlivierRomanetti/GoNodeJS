...
const requestHandler = (req, res) => {
  ...
  if (url === '/') {
    res.write(`
      ...
      `);
    // EXEMPLE RUNTIME ERROR 
    // pour exemple  au runtime enlever le return ci dessous
    return res.end();
  }
  if (url === '/register' && method === 'POST') {
    ...
    return req.on('end', () => {
      ...
      // EXEMPLE LOGICAL ERROR 
      // pour exemple on met 0 au lieu de 1, pour créer une erreur sémantique => on voit que dans extraterrestrials.txt on n'a pas ce que l'on souhaite avoir
      // Faire un break point
